...
object GuildBehavior {
  ...
  def behavior(
    ...
    ZIO.serviceWithZIO[RedisCommands[Task, String, String]](
      redis => // TODO THIS WOULD BE A GOOD PLACE TO INTERACT WITH CASSANDRA JOURNAL? READ EVENTS?
          ...
          .flatMap(members =>
            if (members.size >= 5)
              ...
              (redis.lPush(
                entityId,
                userId
              ) *> // TODO THIS WOULD BE A GOOD PLACE TO INTERACT WITH CASSANDRA JOURNAL? INSERT EVENT?
